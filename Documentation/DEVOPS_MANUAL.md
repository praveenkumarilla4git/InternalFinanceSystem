📘 Operations Manual: Internal Finance System
...
📘 PART 5: SCALING & ORCHESTRATION (FROM 1 SERVER TO 3)
...
📘 PART 6: STATE MANAGEMENT & OPTIMIZATION (S3 BACKEND)
...
📘 PART 7: LOAD BALANCING (THE "TRAFFIC COP")
Objective: Replace direct server IP access with a single Application Load Balancer (ALB) that automatically distributes traffic to healthy servers and handles failures.

Phase 24: Infrastructure Upgrade (Terraform)
We modified main.tf to add three critical components:
1. Target Group: A logical group where we register our 3 servers (Port 5000).
2. Load Balancer (ALB): The public-facing entry point (Port 80).
3. Listener: A rule that forwards traffic from the ALB (Port 80) to the Target Group (Port 5000).

Key Code Changes:
- Security Group: Opened Port 80 (HTTP) to the world.
- Data Sources: Added aws_vpc and aws_subnets lookups so the ALB knows where to live.
- Outputs: Added alb_dns_name to print the final URL.

Phase 25: Deployment & Verification
1. Apply Infrastructure:
   Command: terraform apply
   Result: Terraform creates the ALB and automatically updates inventory.ini with the new Server IPs.

2. Trigger Deployment:
   Command: git push
   Action: GitHub Actions detects the code change, reads the updated inventory.ini, and deploys the Docker container to the new fleet.
...
2. Update: 
   python update_link.py (Updates Local README).
...
Result: Users always find the active link on the GitHub Homepage, regardless of how often the infrastructure changes.
